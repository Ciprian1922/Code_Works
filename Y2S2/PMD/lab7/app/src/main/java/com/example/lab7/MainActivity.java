...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button showStudentsButton = findViewById(R.id.showStudentsButton);
        showStudentsButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                // Call queryAllFriends() when the button is clicked
                queryAllFriends();
            }
        });
    ...
    private void showAddDialog() {
        ...
        addFriend("Popa Ioan-Ciprian", "19.01.2003", "Timisoara");
        addFriend("Popa Roaxna-Maria", "10.10.2006", "Timisoara");
        addFriend("Frincu Andrei", "22.07.2003", "Brasov");
        addFriend("Roxana Patricia", "22.01.2003", "Resita");
    ...
    private void addFriend(String name, String birthday, String address) {
        ...
        cv.put(DBContract.Friends.COLUMN_NAME, name);
        cv.put(DBContract.Friends.COLUMN_BIRTHDAY, birthday);
        cv.put(DBContract.Friends.COLUMN_ADDRESS, address);

        long result = mDatabase.insert(DBContract.Friends.TABLE_NAME, null, cv);
        if (result == -1) {
    ...
    private void queryFriendsByAge(int age) {
        if (age == 0) {
            ...
            Cursor cursor = mDatabase.query(
                    DBContract.Friends.TABLE_NAME,
                    null,
                    "substr(" + DBContract.Friends.COLUMN_BIRTHDAY + ", 7) BETWEEN ? AND ?", // Extracting year part of the birthday
    ...
    private void deleteFriend(String name) {
        ...
        mDatabase.delete(DBContract.Friends.TABLE_NAME,
                DBContract.Friends.COLUMN_NAME + " = ?",
                new String[]{name});
    ...
    private void updateFriend(String originalName, String modifiedName, String modifiedBirthday, String modifiedAddress) {
        ContentValues cv = new ContentValues();
        cv.put(DBContract.Friends.COLUMN_NAME, modifiedName);
        cv.put(DBContract.Friends.COLUMN_BIRTHDAY, modifiedBirthday);
        cv.put(DBContract.Friends.COLUMN_ADDRESS, modifiedAddress);

        // Update the friend entry in the database
        mDatabase.update(DBContract.Friends.TABLE_NAME, cv,
                DBContract.Friends.COLUMN_NAME + " = ?",
    ...
    private void displayFriends(Cursor cursor) {
        ...
        while (cursor.moveToNext()) {
            @SuppressLint("Range") String name = cursor.getString(cursor.getColumnIndex(DBContract.Friends.COLUMN_NAME));
            @SuppressLint("Range") String birthday = cursor.getString(cursor.getColumnIndex(DBContract.Friends.COLUMN_BIRTHDAY));
            @SuppressLint("Range") String address = cursor.getString(cursor.getColumnIndex(DBContract.Friends.COLUMN_ADDRESS));
